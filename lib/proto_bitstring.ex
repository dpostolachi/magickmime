defimpl Magickmime, for: BitString  do
  ...
  defp match_mime( :video, <<0x1A, 0x45, 0xDF, 0xA3, 0x42, 0x82, _, _, 0x77, 0x65, 0x62, 0x6D>> <> _rest ), do: :webm
  defp match_mime( :video, <<_, _, _, _, 0x66, 0x74, 0x79, 0x70, 0x6D, 0x70, 0x34>> <> _rest ), do: :mp4
  defp match_mime( :video, <<_, _, _, _, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6F, 0x6D>> <> _rest ), do: :mp4
  defp match_mime( :video, <<_, _, _, _, 0x66, 0x74, 0x79, 0x70, 0x71, 0x74, 0x20, 0x20>> <> _rest ), do: :mov
  defp match_mime( :video, <<_, _, _, _, 0x66, 0x74, 0x79, 0x70, 0x6D, 0x6F, 0x6F, 0x76>> <> _rest ), do: :mov
  defp match_mime( :video, <<0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11>> <> _rest ), do: :wmv
  defp match_mime( :video, <<0x1A, 0x45, 0xDF, 0xA3, 0x42, 0x82, _, _, 0x6D, 0x61, 0x74, 0x72>> <> _rest ), do: :mkv
  defp match_mime( :video, <<0x1A, 0x45, 0xDF, 0xA3, 0x42, 0x82, _, _, 0x76, 0x69, 0x64, 0x65>> <> _rest ), do: :mkv
  defp match_mime( :video, <<0x1A, 0x45, 0xDF, 0xA3, 0x42, 0x82, _, _, 0x6D, 0x65, 0x74, 0x61>> <> _rest ), do: :mkv
